...
function initIntroPage() {
    // 获取"开始测评"按钮
    const startBtn = document.querySelector('.start-btn-large');
    
    if (!startBtn) {
        console.warn('未找到"开始测评"按钮');
        return;
    }
    ...
    // 隐藏加载遮罩（如果存在）
    if (Navigation && Navigation.hideLoading) {
        Navigation.hideLoading();
    }

    console.log('介绍页初始化完成');
}
...
async function handleStartTest() {
    ...
    // 只有验证和startTest都成功，才跳转到类型选择页
    // 构建选择页URL参数（需要包含token以便后续页面使用）
    const params = {};
    ...
    Navigation.navigateTo('pages/select.html', params);
}
