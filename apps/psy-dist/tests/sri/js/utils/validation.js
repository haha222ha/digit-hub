...
export const validateAnswers = (answers) => {
  ...
  if (answers.length !== 50) {
    return { 
      valid: false, 
      error: `答案数量不正确：应为50题，实际为${answers.length}题` 
    };
  }
...
export const validateQuestions = (questions) => {
  ...
  if (questions.length !== 50) {
    return { valid: false, error: `SRI必须包含50道题目，当前有${questions.length}道` };
  }
...
export const validateOptions = (options) => {
  ...
  // SRI应该有5个选项
  if (options.length !== 5) {
    return { valid: false, error: `SRI应该有5个选项，当前有${options.length}个` };
  }
